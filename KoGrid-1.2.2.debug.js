...
var ROW_KEY = '__kg_rowIndex__';
var GRID_KEY = '__koGrid__';
var ROW_TEMPLATE = 'kgRowTemplate';
var HEADERROW_TEMPLATE = 'kgHeaderRowTemplate';
var HEADERCELL_TEMPLATE = 'kgHeaderCellTemplate';
var FOOTER_TEMPLATE = 'kgFooterTemplate';
var SORT_ASC = "asc"; // constant for sorting direction
var SORT_DESC = "desc"; // constant for sorting direction
...
/***********************************************
* FILE: ..\Src\GridClasses\RowSubscription.js
***********************************************/
kg.RowSubscription = function () {
    var self = this;
    self.rowKey = undefined;
    self.rowIndex = undefined;
    self.node = undefined;
...
kg.RowManager = function (grid) {
    ...
    self.dataSource = grid.finalData; //observableArray
    ...
    self.buildRowFromEntity = function (entity, rowIndex, pagingOffset) {
...
kg.FilterManager = function (options) {
    ...
    var filterData = function () {
        ...
        internalFilteredData(ko.utils.arrayFilter(data, function (item) {
            var propPath,
                i;
...
kg.buildCSSStyles = function (grid) {
    ...
    for (; i < len; i++) {
        ...
        css.append(".{0} .col{1} { left: {2}px; right: {3}px; }", gridId, i, sumWidth, (grid.totalRowWidth() - sumWidth - colWidth));
        sumWidth += colWidth;
    }
    if (kg.utils.isIe) { // IE
...
kg.domUtility = (new function () {
    ...
    this.measureElementMinDims = function ($container) {
        ...
        $testContainer.empty();
        $testCon.append($("<div style='height: 0x; width: 0px;'></div>"));
        dims.minWidth = $testCon.width();
        dims.minHeight = $testCon.height();
        var pixelStr;
        if (!dims.minWidth) {
            pixelStr = $testCon.css("min-width");
            dims.minWidth = parsePixelString(pixelStr);
        }
        if (!dims.minHeight) {
            pixelStr = $testCon.css("min-height");
